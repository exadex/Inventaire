...
test("résumé des trois critères de références persisté",()=>{const st=new MemoryStorage(),report={...audit("references",3),auditType:"references",referenceSummary:{differentDuplicates:1,missing:1,unregisteredInContacts:1,total:3,uniqueItemCount:3}};ok(S.saveWorkspace({audits:[report],sessions:[]},st).ok);ok(S.loadWorkspace(st).audits[0].referenceSummary.unregisteredInContacts===1);});
