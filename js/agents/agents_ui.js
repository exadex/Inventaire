(function () {
  ...
  function renderAuditConfig(root) {
    ...
    root.innerHTML = `<button class="agent-back ghost-btn" data-agent-action="home">← Agents</button><header class="agents-header"><div><p class="eyebrow">Strictement en lecture seule</p><h2>Nouvel audit</h2><p>Aucune fonction d’écriture n’est accessible à l’Auditeur.</p></div></header>
      ...
      ${select("category","Catégorie", window.inventoryCategories || [])}${select("location","Localisation",window.inventoryLocations || [])}${select("type","Type",Array.from(new Set(currentAuditItems().map(item=>item.type||item.inventoryType||item.kind||"Produit"))))}</div><section class="audit-explanation" aria-live="polite"><h3>Explication</h3><p>${esc(explanations.full)}</p></section><div class="modal-actions"><button class="primary-btn" type="submit">Analyser en lecture seule</button></div></form>`;
    bind(root);
  ...
  function renderAuditReport(root) {
    ...
    const filters=audit.filters||{},filterSummary=[["Catégorie",filters.category],["Localisation",filters.location],["Type",filters.type]].filter(([,value])=>value&&value!=="all").map(([label,value])=>`${label} : ${value}`).join(" · ")||"Tout l’inventaire";
    root.innerHTML = `<button class="agent-back ghost-btn" data-agent-action="home">← Agents</button><header class="agents-header"><div><p class="eyebrow">Rapport d’audit</p><h2>Audit du ${fmt(audit.createdAt)}</h2><p>${audit.itemCount} items analysés en ${audit.durationMs} ms.</p><p>Périmètre : ${esc(filterSummary)}</p>${audit.itemCount===0?`<p class="agent-legacy-notice">Aucun article ne correspond au périmètre sélectionné.</p>`:""}${fullSummary}${(audit.rulesVersion||1)<3?`<p class="agent-legacy-notice">Ce rapport utilise une ancienne méthode de calcul. Relancez l’analyse pour obtenir le nouveau Score de doublon.</p>`:""}</div><button class="primary-btn" data-agent-action="audit">Relancer l’analyse</button></header>
