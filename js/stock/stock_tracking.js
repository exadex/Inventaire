(function () {
  ...
  const VERSION = 1;
  const TYPES = new Set(["received", "container_opened", "consumed", "recounted", "moved", "container_finished", "aliquots_prepared", "aliquots_consumed", "aliquots_moved", "aliquot_opened", "open_aliquot_consumed", "open_aliquot_moved", "open_aliquot_discarded", "preparation_recounted", "preparation_finished", "corrected", "configuration_changed"]);
  ...
  function normalizeTracking(item = {}) {
    ...
    return {
      ...
      openContainers: (Array.isArray(raw.openContainers) ? raw.openContainers : []).map(container => ({ ...container, id: container.id || id("container"), remaining: Math.max(0, number(container.remaining)), status: container.status === "finished" ? "finished" : "open", version: Math.max(1, Math.trunc(number(container.version) || 1)) }))
    };
  }
  ...
  function totalClosed(tracking) { return tracking.closedByLocation.reduce((sum, row) => sum + row.quantity, 0); }
  ...
  function available(item) {
    ...
    const closed = totalClosed(tracking);
  ...
  function eventFor(item, operation, user) { const now = new Date().toISOString(); return { id: id("movement"), operationId: operation.operationId, itemId: item.id, timestamp: now, ...actor(user), type: TYPES.has(operation.type) ? operation.type : "corrected", entityType: operation.entityType || "item", entityId: operation.entityId || "", preparationId: operation.preparationId || "", before: operation.before ?? null, after: operation.after ?? null, quantity: number(operation.quantity), unit: operation.unit || "", sourceEquivalentConsumed: number(operation.sourceEquivalentConsumed), sourceUnit: operation.sourceUnit || "", fromLocation: operation.fromLocation || "", toLocation: operation.toLocation || "", comment: operation.comment || "", correctionReason: operation.correctionReason || "" }; }
  ...
  function apply(itemInput, operation, user = {}) {
    ...
    if (operation.type === "received") { if (tracking.mode === "simple") { const unit=normalizeUnitLabel(item.unit),qty=validateUnitQuantity(operation.quantity,unit);if(qty<=0)throw new Error("La quantité reçue doit être positive.");before=number(item.quantity);item.quantity=round(before+qty);after=item.quantity;operation.unit=unit.plural; } else { const qty = validateStep(operation.quantity, tracking, true), row = rowAt(tracking.closedByLocation, operation.toLocation); before = row.quantity; row.quantity += qty; after = row.quantity; } }
    else if (operation.type === "moved" && operation.entityType === "closed") { const qty = validateStep(operation.quantity, tracking, true), from = rowAt(tracking.closedByLocation, operation.fromLocation), to = rowAt(tracking.closedByLocation, operation.toLocation); if (from.quantity < qty) throw new Error("Quantité fermée insuffisante dans cette localisation."); before = { from: from.quantity, to: to.quantity }; from.quantity -= qty; to.quantity += qty; after = { from: from.quantity, to: to.quantity }; }
    else if (operation.type === "container_opened") { const source = rowAt(tracking.closedByLocation, operation.fromLocation); if (source.quantity < 1) throw new Error("Aucun contenant fermé n’est disponible dans cette localisation."); source.quantity -= 1; const sequence = tracking.openContainers.length + 1; const container = { id: id("container"), label: `${tracking.packagingLevels[0].singular} ouvert nº${sequence}`, location: operation.toLocation || operation.fromLocation, remaining: capacity(tracking), capacity: capacity(tracking), status: "open", openedAt: now, openedBy: actor(user), updatedAt: now, updatedBy: by, version: 1 }; tracking.openContainers.push(container); operation.entityId = container.id; operation.entityType = "container"; after = clone(container); }
    ...
    tracking.closedByLocation = tracking.closedByLocation.filter(row => row.quantity);
