(function () {
  ...
  const hasNumericRemaining = container => typeof container?.remaining === "number" && Number.isFinite(container.remaining);
  const isEffectivelyEmpty = container => hasNumericRemaining(container) && container.remaining <= ZERO_REMAINING_TOLERANCE;
  function finishEmptyContainer(container, now = new Date().toISOString()) {
    if (!isEffectivelyEmpty(container) || container.status === "finished") return false;
    container.remaining = 0; container.status = "finished"; container.finishedAt = container.finishedAt || now;
    return true;
  }
  ...
  function normalizeTracking(item = {}) {
    ...
    const normalized = {
      ...
      openContainers: (Array.isArray(raw.openContainers) ? raw.openContainers : []).map(container => {
        ...
        const explicit=hasNumericRemaining(container),remaining=explicit?Math.min(containerCapacity,Math.max(0,container.remaining)):container.remaining;
        return { ...container, id: container.id || id("container"), remaining,capacity:containerCapacity,unitKey:container.unitKey||trackingLevel.key,status: container.status === "finished" || (explicit && container.remaining <= ZERO_REMAINING_TOLERANCE) ? "finished" : "open", version: Math.max(1, Math.trunc(number(container.version) || 1)) };
      })
    };
    ...
    normalized.closedContainers = Array.isArray(raw.closedContainers)
      ? raw.closedContainers.map((container,index)=>{
        ...
        const explicit=hasNumericRemaining(container),remaining=explicit?Math.min(containerCapacity,Math.max(0,container.remaining)):(container.remaining??containerCapacity);
        return { ...container, id:container.id||`closed-${stablePart(item.id)}-${index+1}`, label:container.label||`${outer.singular} fermé nº${index+1}`, location:String(container.location||item.location||""), remaining,capacity:containerCapacity,unitKey:container.unitKey||normalized.trackingUnitKey,status:container.status==="finished"||(explicit&&container.remaining<=ZERO_REMAINING_TOLERANCE)?"finished":"closed",version:Math.max(1,Math.trunc(number(container.version)||1)) };
      })
    ...
    if(Array.isArray(raw.closedContainers)){const rows=new Map();normalized.closedContainers.filter(row=>row.status==="closed").forEach(container=>{if(container.location)rows.set(container.location,(rows.get(container.location)||0)+1)});normalized.closedByLocation=[...rows].map(([location,quantity])=>({location,quantity}));}
    return normalized;
  }

  function normalizeAliquotsBase(item = {}) {
  ...
  function normalizeAliquots(item={}){const normalized=normalizeAliquotsBase(item),raw=item.aliquotTracking?.preparations||[];normalized.preparations.forEach(prep=>{const source=raw.find(row=>row.id===prep.id);prep.locations.forEach((location,index)=>Object.assign(location,{roomId:source?.locations?.[index]?.roomId||"",locationId:source?.locations?.[index]?.locationId||"",sublocationId:source?.locations?.[index]?.sublocationId||""}));});return normalized;}
  ...
  function totalClosed(tracking) { return Array.isArray(tracking.closedContainers) ? tracking.closedContainers.filter(row=>row.status==="closed").length : tracking.closedByLocation.reduce((sum, row) => sum + row.quantity, 0); }
  ...
  function available(item) {
    ...
    const closed = Array.isArray(tracking.closedContainers) ? tracking.closedContainers.filter(row=>row.status==="closed"&&hasNumericRemaining(row)).reduce((sum,row)=>sum+Math.max(0,row.remaining)/cap,0) : totalClosed(tracking);
    const opened = tracking.openContainers.filter(row => row.status === "open" && hasNumericRemaining(row)).reduce((sum, row) => sum + Math.max(0,row.remaining) / cap, 0);
  ...
  function unitDimension(levelOrKey) {
    const isLevel = levelOrKey && typeof levelOrKey === "object";
    const key = String(isLevel ? levelOrKey.key || "" : levelOrKey || "");
    if (["L", "mL", "µL"].includes(key)) return "volume";
    if (["g", "mg", "µg", "ng"].includes(key)) return "mass";
    if (["mol", "mmol", "µmol"].includes(key)) return "amount";
    if (["M", "mM", "µM", "nM"].includes(key)) return "concentration";
    return isLevel && levelOrKey.kind === "discrete" ? "discrete" : "unknown";
  }
  // échelle métrique relative à l'unité de base de chaque dimension (L, g, mol, M) pour convertir entre préfixes (mL/µL, mg/g...)
  const METRIC_UNIT_SCALE = {
    L: 1, mL: 1e-3, "µL": 1e-6,
    g: 1, mg: 1e-3, "µg": 1e-6, ng: 1e-9,
    mol: 1, mmol: 1e-3, "µmol": 1e-6,
    M: 1, mM: 1e-3, "µM": 1e-6, nM: 1e-9
  };
  function metricConversionFactor(fromKey, toKey) {
    if (fromKey === toKey) return 1;
    const dimension = unitDimension(fromKey);
    if (dimension === "unknown" || dimension === "discrete" || dimension !== unitDimension(toKey)) return null;
    const fromScale = METRIC_UNIT_SCALE[fromKey], toScale = METRIC_UNIT_SCALE[toKey];
    if (!Number.isFinite(fromScale) || !Number.isFinite(toScale)) return null;
    return fromScale / toScale;
  }
  // unité de référence d'un item pour comparer aux protocoles : le dernier niveau de conditionnement (3e, sinon 2e, sinon 1er)
  function referenceUnit(item) {
    return normalizeTracking(item).packagingLevels.at(-1);
  }
  // convertit une quantité exprimée dans l'unité de référence (dernier niveau) vers l'unité native de l'item (premier niveau, celle de item.quantity)
  function toOuterQuantity(item, valueAtReferenceUnit) {
    return round(number(valueAtReferenceUnit) / capacity(normalizeTracking(item)));
  }
  function equivalentLevels(item, canonicalTotal = available(item)) {
    const tracking = normalizeTracking(item), levels = tracking.packagingLevels, total = number(canonicalTotal), results = [], seen = new Set();
    if (!Number.isFinite(total) || total < 0) return results;
    let converted = total, validChain = true;
    levels.forEach((level, index) => {
      if (!validChain) return;
      if (index) {
        const previous = levels[index - 1], previousDimension = unitDimension(previous), dimension = unitDimension(level);
        if (previous.kind === "continuous" && level.kind === "continuous" && (previousDimension === "unknown" || dimension === "unknown" || previousDimension !== dimension)) { validChain = false; return; }
        const factor = Number(level.contains);
        if (!Number.isFinite(factor) || factor <= 0) { validChain = false; return; }
        converted = round(converted * factor);
      }
      const identity = String(level.key || level.singular || level.plural).toLocaleLowerCase("fr-FR");
      if (!identity || seen.has(identity) || !Number.isFinite(converted)) return;
      seen.add(identity);
      results.push({ key:level.key, value:converted, singular:level.singular, plural:level.plural, kind:level.kind });
    });
    return results;
  }
  ...
  function syncClosedSummary(tracking) {
    const rows = new Map();
    (tracking.closedContainers || []).filter(container=>container.status==="closed").forEach(container => {
  ...
  function repairZeroRemainingContainers(itemInput, existingMovements = []) {
    const item=clone(itemInput),raw=item.stockTracking||{},openContainers=Array.isArray(raw.openContainers)?clone(raw.openContainers):[],closedContainers=Array.isArray(raw.closedContainers)?clone(raw.closedContainers):null,known=new Set((existingMovements||[]).flatMap(row=>[row?.id,row?.operationId]).filter(Boolean)),events=[];let changed=false;
    const repair=(container,activeStatus)=>{
      if(!container?.id||container.status==="finished"||(container.status&&container.status!==activeStatus)||!isEffectivelyEmpty(container))return;
      const operationId=`repair-zero-${item.id}-${container.id}`;container.remaining=0;container.status="finished";container.finishedAt=container.finishedAt||container.updatedAt||item.updatedAt||new Date().toISOString();changed=true;
      if(known.has(operationId))return;known.add(operationId);
      const tracking=normalizeTracking(item),unit=tracking.packagingLevels.find(level=>level.key===(container.unitKey||tracking.trackingUnitKey))||trackingLevel(tracking),event=eventFor(item,{operationId,type:"automatic_repair",entityType:"container",entityId:container.id,containerId:container.id,containerLabel:container.label||container.id,containerStatusBefore:activeStatus,containerStatusAfter:"finished",beforeQuantity:0,afterQuantity:0,unit:unit.plural,before:{...container,status:activeStatus},after:container,automatic:true,automaticFinish:true}, {name:"Système"});
      Object.assign(event,{id:operationId,operationId,timestamp:container.finishedAt,userId:"system",userName:"Système",userEmoji:"",userInitials:"SY"});events.push(event);
    };
    openContainers.forEach(container=>repair(container,"open"));closedContainers?.forEach(container=>repair(container,"closed"));
    if(!changed)return{item:itemInput,events:[],changed:false};
    item.stockTracking={...raw,openContainers,...(closedContainers?{closedContainers}:{})};
    if(closedContainers){const rows=new Map();closedContainers.filter(row=>row.status==="closed").forEach(container=>{if(container.location)rows.set(container.location,(rows.get(container.location)||0)+1)});item.stockTracking.closedByLocation=[...rows].map(([location,quantity])=>({location,quantity}));}
    item.quantity=available(item);
    return{item,events,changed:true};
  }
  ...
  function applyContainerRecount(itemInput, operation, user = {}) {
    ...
    changes.forEach(change=>{
      ...
      if(change.isNew){
        ...
        const containerId=id("closed-container"),container={id:containerId,label:`${outer.singular} ${status==="closed"?"fermé":"ouvert"}`,location,roomId:change.roomId||"",locationId:change.locationId||"",sublocationId:change.sublocationId||"",remaining,capacity:containerCapacity,unitKey:selectedLevel.key,status,createdAt:now,createdBy:actor(user),updatedAt:now,updatedBy:by,version:1};finishEmptyContainer(container,now);
        ...
        const finalDisplay=round(container.remaining/factor);events.push(eventFor(item,{...operation,operationId:`${operation.operationId}-${containerId}`,type:"received",entityType:"container",entityId:containerId,containerId,containerLabel:container.label,containerStatusBefore:"",containerStatusAfter:container.status,beforeQuantity:0,afterQuantity:finalDisplay,beforeCapacity:0,afterCapacity:displayCapacity,unitAfter:selectedLevel.plural,difference:finalDisplay,quantity:finalDisplay,unit:selectedLevel.plural,toLocation:location,before:null,after:container,comment:operation.comment||""},user));return;
      }
      ...
      const next={...container,location,roomId:change.roomId||container.roomId||"",locationId:change.locationId||container.locationId||"",sublocationId:change.sublocationId||"",remaining,capacity:containerCapacity,unitKey:selectedLevel.key,status,updatedAt:now,updatedBy:by,version:container.version+1};const automaticallyFinished=finishEmptyContainer(next,now);
      if(next.status==="finished"){next.finishedAt=next.finishedAt||now;if(next.remaining!==0)throw new Error(`Un contenant terminé doit avoir une quantité restante égale à zéro.`);}
      if(next.status==="open"&&!next.openedAt){next.openedAt=now;next.openedBy=actor(user);}
      if(sourceType==="closed")tracking.closedContainers.splice(index,1);else tracking.openContainers.splice(index,1);
      if(next.status==="closed")tracking.closedContainers.push(next);else if(next.status==="open")tracking.openContainers.push(next);else if(sourceType==="closed")tracking.closedContainers.push(next);else tracking.openContainers.push(next);
      const beforeLevel=tracking.packagingLevels.find(level=>level.key===(before.unitKey||tracking.trackingUnitKey))||trackingLevel(tracking),beforeIndex=tracking.packagingLevels.findIndex(level=>level.key===beforeLevel.key),beforeFactor=tracking.packagingLevels.slice(beforeIndex+1).reduce((value,level)=>value*level.contains,1),beforeDisplay=round(before.remaining/beforeFactor),beforeCapacity=round(before.capacity/beforeFactor),afterDisplay=round(next.remaining/factor),eventOperation={...operation,operationId:`${operation.operationId}-${container.id}`,type:"recounted",entityType:"container",entityId:container.id,containerId:container.id,containerLabel:container.label||container.id,containerStatusBefore:before.status,containerStatusAfter:next.status,beforeQuantity:beforeDisplay,afterQuantity:afterDisplay,beforeCapacity,afterCapacity:displayCapacity,unitBefore:beforeLevel.plural,unitAfter:selectedLevel.plural,difference:round((next.remaining-before.remaining)/factor),quantity:afterDisplay,unit:selectedLevel.plural,fromLocation:before.location||"",toLocation:location,before,after:next,automatic:automaticallyFinished,automaticFinish:automaticallyFinished,comment:operation.comment||""};
  ...
  function apply(itemInput, operation, user = {}) {
    ...
    aliquots.preparations.forEach(prep=>{const raw=(item.aliquotTracking?.preparations||[]).find(row=>row.id===prep.id);prep.locations.forEach((location,index)=>Object.assign(location,{roomId:raw?.locations?.[index]?.roomId||"",locationId:raw?.locations?.[index]?.locationId||"",sublocationId:raw?.locations?.[index]?.sublocationId||""}));});
    const simpleAliquotEquivalentBefore = tracking.mode === "simple" ? activeAliquotEquivalent(item) : 0;
    let simpleSourceUsed = 0;
    const findContainer = () => { const entity = tracking.openContainers.find(row => row.id === (operation.sourceId || operation.entityId) && row.status === "open"); if (!entity) throw new Error("Ce contenant n’est plus disponible."); if (operation.expectedVersion && entity.version !== operation.expectedVersion) throw new Error("CONFLICT: ce contenant a été modifié. Rechargez les données."); return entity; };
    ...
    if(operation.type==="aliquots_prepared"&&operation.sourceType==="closed"){
      const sourceIndex=tracking.closedContainers.findIndex(row=>row.id===operation.sourceId&&row.status==="closed");if(sourceIndex<0)throw new Error("Ce contenant fermé n’est plus disponible.");
      const sourceQuantityInput=parseLocalizedNumber(operation.sourceQuantity),displayQty=validateUnitQuantity(sourceQuantityInput,trackingLevel(tracking)),qty=toBaseQuantity(displayQty,tracking),closed=tracking.closedContainers[sourceIndex];if(qty<=0||qty>closed.remaining)throw new Error("Stock source insuffisant.");
      tracking.closedContainers.splice(sourceIndex,1);const source={...closed,status:"open",openedAt:now,openedBy:actor(user),updatedAt:now,updatedBy:by,version:closed.version+1};tracking.openContainers.push(source);operation.sourceType="container";operation.entityId=source.id;operation.containerStatusBefore="closed";operation.containerStatusAfter=qty===closed.remaining?"finished":"open";
    }
    if(operation.type==="preparation_corrected"){
      const prep=findPrep(),beforePrep=clone(prep),target=Math.max(0,Math.trunc(parseLocalizedNumber(operation.availableCount))),openCount=(prep.openAliquots||[]).filter(row=>row.status==="open").length;if(target<openCount)throw new Error("Le nombre disponible ne peut pas être inférieur au nombre d’aliquotes ouvertes.");
      prep.locations=[{location:operation.location||prep.locations[0]?.location||"",roomId:operation.roomId||"",locationId:operation.locationId||"",sublocationId:operation.sublocationId||"",quantity:target-openCount}].filter(row=>row.location&&row.quantity);prep.volume=Math.max(0,number(operation.volume));prep.volumeUnit=operation.volumeUnit||prep.volumeUnit||"";prep.concentration=Math.max(0,number(operation.concentration));prep.concentrationUnit=operation.concentrationUnit||prep.concentrationUnit||"";prep.status=target===0?"finished":"active";if(target===0)prep.finishedAt=now;prep.updatedAt=now;prep.updatedBy=by;prep.version+=1;
      operation.preparationId=prep.id;operation.beforeQuantity=remainingAliquots(beforePrep)+openCount;operation.afterQuantity=target;operation.difference=target-operation.beforeQuantity;operation.quantity=target;operation.unit="aliquotes";operation.before=beforePrep;operation.after=clone(prep);syncClosedSummary(tracking);item.stockTracking=tracking;item.aliquotTracking=aliquots;item.quantity=available(item);const event=eventFor(item,operation,user);return{item,event,events:[event]};
    }
    if (operation.type === "received") { if (tracking.mode === "simple") { const unit=normalizeUnitLabel(item.unit),qty=validateUnitQuantity(operation.quantity,unit);if(qty<=0)throw new Error("La quantité reçue doit être positive.");before=number(item.quantity);item.quantity=round(before+qty);after=item.quantity;operation.unit=unit.plural;operation.difference=qty; } else { const qty = validateStep(operation.quantity, tracking, true), row = rowAt(tracking.closedByLocation, operation.toLocation); before = row.quantity;for(let index=0;index<qty;index+=1)tracking.closedContainers.push({id:id("closed-container"),label:`${tracking.packagingLevels[0].singular} fermé nº${tracking.closedContainers.length+1}`,location:operation.toLocation,remaining:capacity(tracking),capacity:capacity(tracking),unitKey:tracking.trackingUnitKey,status:"closed",createdAt:now,updatedAt:now,updatedBy:by,version:1});row.quantity += qty; after = row.quantity;operation.difference=qty; } }
    ...
    else if (["consumed", "recounted", "container_finished", "moved"].includes(operation.type) && operation.entityType === "container") { const container = findContainer(); before = clone(container);operation.containerId=container.id;operation.containerLabel=container.label;operation.containerStatusBefore=container.status; if (operation.type === "consumed") { const displayQty = validateUnitQuantity(operation.quantity, trackingLevel(tracking)), qty = toBaseQuantity(displayQty, tracking); if (qty > container.remaining + ZERO_REMAINING_TOLERANCE) throw new Error("La quantité demandée dépasse le contenu disponible."); operation.beforeQuantity=fromBaseQuantity(container.remaining,tracking);container.remaining = round(container.remaining - qty);operation.afterQuantity=fromBaseQuantity(Math.max(0,container.remaining),tracking); operation.unit = trackingLevel(tracking).plural;operation.difference=round(operation.afterQuantity-operation.beforeQuantity);if(finishEmptyContainer(container,now)){operation.afterQuantity=0;operation.automatic=true;operation.automaticFinish=true;} } else if (operation.type === "recounted") { const displayQty = validateUnitQuantity(operation.quantity, trackingLevel(tracking)), qty = toBaseQuantity(displayQty, tracking); if (qty > container.capacity) throw new Error("Le comptage dépasse la capacité du contenant.");operation.beforeQuantity=fromBaseQuantity(container.remaining,tracking); container.remaining = qty;operation.afterQuantity=displayQty; operation.unit = trackingLevel(tracking).plural;operation.difference=fromBaseQuantity(qty-before.remaining,tracking);if(finishEmptyContainer(container,now)){operation.afterQuantity=0;operation.automatic=true;operation.automaticFinish=true;} } else if (operation.type === "moved") container.location = operation.toLocation; else { if (container.remaining > ZERO_REMAINING_TOLERANCE && !operation.correctionReason) throw new Error("Une justification est obligatoire pour terminer un contenant non vide."); container.remaining=Math.max(0,container.remaining);container.status = "finished"; container.finishedAt = now; } container.updatedAt = now; container.updatedBy = by; container.version += 1; after = clone(container);operation.containerStatusAfter=container.status; }
      ...
      const prep = { id: id("preparation"), label: `Préparation nº${aliquots.preparations.length + 1}`, createdCount: count, representedSourceQuantity: represented, sourceUnit: operation.sourceUnit || tracking.trackingUnit, volume: number(operation.volume), volumeUnit: operation.volumeUnit || "", concentration: number(operation.concentration), concentrationUnit: operation.concentrationUnit || "", locations, openAliquots: [], preparedAt: operation.preparedAt || now, preparedBy: actor(user), sourceContainerId: operation.sourceId || "", note: operation.comment || "", status: "active", updatedAt: now, updatedBy: by, version: 1 }; aliquots.preparations.push(prep); operation.entityId = prep.id; operation.entityType = "preparation"; operation.quantity = count; operation.unit = "aliquotes";if(operation.sourceType==="container"){const source=tracking.openContainers.find(row=>row.id===operation.sourceId);if(source&&finishEmptyContainer(source,now)){operation.containerId=source.id;operation.containerLabel=source.label;operation.containerStatusBefore="open";operation.containerStatusAfter="finished";operation.automatic=true;operation.automaticFinish=true;}} after = clone(prep); }
  ...
  window.StockTracking = { VERSION, ZERO_REMAINING_TOLERANCE, id, normalizeUnitLabel, parseLocalizedNumber, packagingPreview, migrationComparison, migrationPresentation, normalizeTracking, normalizeAliquots, capacity, trackingLevel, trackingFactor, trackingCapacity, fromBaseQuantity, toBaseQuantity, validateUnitQuantity, totalClosed, remainingAliquots, aliquotEquivalent, activeAliquotEquivalent, activePreparationLabels, simpleRawAvailable, available, equivalentLevels, repairZeroRemainingContainers, format, plural, summary, validateStep, apply, unitDimension, metricConversionFactor, referenceUnit, toOuterQuantity };
})();
