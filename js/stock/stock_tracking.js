(function () {
  ...
  function applyContainerRecount(itemInput, operation, user = {}) {
    ...
    changes.forEach(change=>{
      ...
      const next={...container,location,roomId:change.roomId||container.roomId||"",locationId:change.locationId||container.locationId||"",sublocationId:change.sublocationId||"",remaining,capacity:containerCapacity,unitKey:selectedLevel.key,status,updatedAt:now,updatedBy:by,version:container.version+1};
  ...
  function apply(itemInput, operation, user = {}) {
    ...
    if(operation.type==="aliquots_prepared"&&operation.sourceId){const source=tracking.openContainers.find(row=>row.id===operation.sourceId);if(source&&source.remaining<=0){source.remaining=0;source.status="finished";source.finishedAt=source.finishedAt||now;}}
